...
function des_encode(data, _deskey) { return strEnc(data, _deskey, null, null); }
function des_decode(data, _deskey) { return strDec(data, _deskey, null, null); }
function md5(data) { return hex_md5(data); }
function getEncParams(params, _nowtime, _deskey) {
 var timestamp = _nowtime;
 var token = md5(md5(params)+md5(timestamp));
 var _params = b64_encode(des_encode(params, _deskey));
 _params = "params=" + _params + "&token="+token+"&timestamp="+timestamp;
 return _params;
 }
 function newbee(password,randnumber){
...
function checkpwd1(username,pwd){
      var userzh = username; //取账号
